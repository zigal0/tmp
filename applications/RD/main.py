...
def prepare_custom_markup(scenario=None, spec=specialize_prev_mem_dep, need_interproc_pass=True, config=None):
    ...
    wdir = working_dir
    os.chdir("../../front")
    os.system('./get_thin_graph.sh -i ' + os.path.join(wdir, src) + ' -s '+ os.path.join(wdir, ir_dotfile) + ' -p '+os.path.join(wdir,ir_pic) + ' --ssa')
...
if __name__ == '__main__':
    if (len(sys.argv)<2 or (sys.argv[1] =="--test" and sys.argv[2]=="empty_labeling")):
       ...
       prepare_graph_from_article()
        ...
        scenario = {
                    ...
                    'data':{'yes_df_list': [
                            ...
                            'yes_cf_list': [
                                [
                                    "any assign_phi", 
                                    "assign_phi assign_phi", 
                                    "assign_phi if_cond", 
                                    "assign_struct any", 
                                    "any if_cond", 
                                    "if_cond any",  
                                    "assign_function_call any", 
                                    "any assign_function_call",
                                    "assign_address assign_modified_address",
                                    "assign_modified_address assign_address",
                                    "assign_phi any",
                                    "if_cond assign_struct"
                                ]
                            ],
                            'no_cf_list' : [["return_val exit"]],
                            'rel_kinds'  : set()}
        ...
        def emit_grammar(gr=[['S','AB'],['A','l'],['B','CD'],['C','j'],['D','EF'],['E','b'],['F','i']], path="1.txt"):
    ...
    elif (sys.argv[1]=="--test" and sys.argv[2]=="vector"):
        config = {
            "working_dir" : os.path.join(current_path, "../front"),
            "src" : "1.c",
            "ir_dotfile" : "m.dot",
